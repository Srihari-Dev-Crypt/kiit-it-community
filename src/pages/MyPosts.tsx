...
import {
  FileText, EyeOff, HelpCircle, MessageSquare, Megaphone, Lightbulb, Plus, User,
} from "lucide-react";
  ...
  if (!user) {
    return (
      <div className="container mx-auto px-4 py-16 text-center">
        ...
        <Link to="/login"><Button variant="gradient">Sign In</Button></Link>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="font-display text-2xl font-bold gradient-text-primary mb-1">My Posts</h1>
          <p className="text-muted-foreground text-sm">View and manage all your posts, confessions, and questions</p>
        </div>
        <Link to="/create">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 px-4 rounded-card bg-card border border-border shadow-card">
          <FileText className="h-16 w-16 mx-auto text-muted-foreground mb-4" />
          <h3 className="text-xl font-semibold mb-2">
            {activeFilter === "all" ? "No posts yet" : `No ${activeFilter}s yet`}
          </h3>
          <p className="text-muted-foreground mb-6">
            {activeFilter === "all" ? "Start sharing with the KIIT community!" : `You haven't posted any ${activeFilter}s yet.`}
          </p>
          <Link to="/create">
            <Button variant="gradient" className="gap-2 rounded-full">
