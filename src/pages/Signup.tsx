    ...
    <div className="min-h-screen flex items-center justify-center p-4 py-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="font-display text-2xl md:text-3xl font-bold mb-2">Create your account</h1>
          <p className="text-muted-foreground">Join the community and start sharing</p>
        </div>

        <div className="p-6 md:p-8 rounded-card bg-card border border-border shadow-card">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <div className="space-y-2">
              <Label htmlFor="displayName">Display Name (optional)</Label>
              <div className="relative">
                <User className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
                <Input id="displayName" type="text" placeholder="How should we call you?" className="pl-10" {...register("displayName")} />
              </div>
              {errors.displayName && <p className="text-sm text-destructive">{errors.displayName.message}</p>}
              <p className="text-xs text-muted-foreground">This won't be shown on anonymous posts</p>
            </div>
                ...
                {passwordRequirements.map((req) => {
                  ...
                  return (
                    <div key={req.label} className="flex items-center gap-2 text-xs">
                      ...
                      <span className={isMet ? 'text-success' : 'text-muted-foreground'}>{req.label}</span>
                    </div>
                  );
                })}
              </div>
            ...
            <Button type="submit" variant="gradient" size="lg" className="w-full gap-2" disabled={isLoading}>
          ...
          <Link to="/login">
            <Button variant="outline" size="lg" className="w-full">Sign in instead</Button>
          </Link>
        </div>
