...
import { User, Settings, Shield, LogOut, Loader2, Save } from "lucide-react";
  ...
  const updateProfile = useMutation({
    mutationFn: async (data: ProfileForm) => {
      ...
      const { error } = await supabase.from("profiles").update({
        display_name: data.display_name, bio: data.bio, is_anonymous_default: data.is_anonymous_default,
      }).eq("user_id", user.id);
      if (error) throw error;
    },
    onSuccess: () => {
    ...
    <div className="container mx-auto px-4 py-8 max-w-2xl">
      ...
      <Tabs defaultValue="profile" className="space-y-5">
        ...
        <TabsContent value="profile">
          {isLoading ? (
            ...
            <form onSubmit={handleSubmit((data) => updateProfile.mutate(data))} className="space-y-5">
              ...
              <Button type="submit" variant="gradient" className="w-full gap-2 rounded-full" disabled={!isDirty || updateProfile.isPending}>
        ...
        <TabsContent value="privacy">
          <div className="p-5 rounded-card bg-card border border-border space-y-5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-xs text-muted-foreground">New posts will be anonymous by default</div>
              </div>
          ...
          <Button type="button" variant="gradient" className="w-full gap-2 mt-4 rounded-full" disabled={!isDirty || updateProfile.isPending} onClick={handleSubmit((data) => updateProfile.mutate(data))}>
            {updateProfile.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <><Save className="h-4 w-4" />Save Changes</>}
          </Button>
        </TabsContent>
          ...
          <div className="space-y-4">
            <div className="p-5 rounded-card bg-card border border-border">
              ...
              <div className="space-y-2 text-xs">
                ...
                <div className="flex justify-between"><span className="text-muted-foreground">Member since</span><span>{new Date(user.created_at).toLocaleDateString()}</span></div>
              </div>
            </div>
            <Button variant="destructive" className="w-full gap-2 rounded-full" onClick={handleSignOut}>
