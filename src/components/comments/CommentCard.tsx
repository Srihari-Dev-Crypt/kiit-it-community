...
export function CommentCard({ comment, postId }: CommentCardProps) {
  ...
  return (
    <div className={cn(
      "p-4 rounded-card bg-card border transition-all",
      ...
      <div className="flex gap-3">
        <div className="flex flex-col items-center gap-0.5">
          <Button
            variant="ghost"
            size="icon"
            className={cn("h-7 w-7", vote === 1 && "text-primary bg-primary/10")}
          ...
          <Button
            variant="ghost"
            size="icon"
            className={cn("h-7 w-7", vote === -1 && "text-destructive bg-destructive/10")}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2 text-sm">
            <span className="text-muted-foreground flex items-center gap-1">
              {comment.identity_type === "anonymous" ? <EyeOff className="h-3 w-3" /> : <Eye className="h-3 w-3" />}
              {displayName}
            </span>
